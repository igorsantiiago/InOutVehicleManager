...
public class Handler : IRequestHandler<Request, Response>
{
    ...
    public async Task<Response> Handle(Request request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            await _repository.RemoveRole(employee, role, cancellationToken);
        }
        catch (Exception ex)
        {
            return new Response($"Erro: {ex.Message}", 400);
        ...
        #region Response
        return new Response($"Cargo de perfil {role.Name} removido ao perfil {employee.Name}.", new ResponseData(employee.Id, role.Name));
        #endregion
    }
}